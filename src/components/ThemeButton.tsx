...
export default function ThemeButton() {
    ...
    if (!mounted) {
      return (<button className={"max-md:text-xl text-2xl p-2 rounded-xl h-[40px] w-[40px] border dark:border-gray-400 border-gray-700 my-2"}>
      { <BiLoaderCircle className={"animate-spin"}/> }
  </button>)
    }
    return (<button aria-label={"Change Theme"} onClick={() => setTheme(theme == "dark" ? "light" : "dark")} className={"group h-[40px] w-[40px] border dark:border-gray-400 border-gray-700 dark:hover:text-cyan-400 hover:text-cyan-600 relative transition-all rounded-xl my-2"}>
