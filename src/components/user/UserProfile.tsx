...
export function UserProfile({ tag }: { tag: string }) {
    const searchParams = useSearchParams();
    const [limit, setLimit] = useState(5);
    const { data, status } = useQuery<UserData | undefined, Error>(['userdata', tag], async () => {
        return await fetch(`/api/users?search=${tag}`).then(async (data) => await data.json().then((i) => {
